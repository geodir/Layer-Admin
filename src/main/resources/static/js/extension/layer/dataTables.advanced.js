...
(function(factory) {
        ...
        if (dataCreateUrl) {
            ...
            */
            return {
                ...
                'className': 'btn-action add',
                'name': 'add',
                'text': datatables.i18n('buttons.add', 'Agregar')
            };
            //}
        }
    };
    ...
    function deleteBatchButton(datatables, conf) {
        ...
        if (deleteBatchUrl) {
            return {
                ...
                'name': 'delete',
                'text': datatables.i18n('buttons.delete', 'Eliminar')
            };
        }
    };
     ...
     * the table tag attribute 'data-create-url' to be used as the URL.
     */
    function addBatchButton(datatables, conf) {
        var addBatchUrl = getDataAddBatchUrl(datatables);
        if (addBatchUrl) {
        	console.log('adding batch button');
        	var tableId = getTableId(datatables);
        	console.log(tableId + 'AddBatchButton');
        	$('#' + tableId + 'AddBatchButton').on('click', function() {
             	addBatchElement(datatables);
            });
        }
    };
     ...
     */
    function addBatchElement(datatables) {
        var $token = $("meta[name='_csrf']");
        var $header = $("meta[name='_csrf_header']");
        var rows_selected = datatables.columns().checkboxes.selected();
        var tableId = getTableId(datatables);
        var rowIds = rows_selected.join(",");
        var url = getAddBatchUrl(datatables, rowIds);
        $.ajax({
                url: url,
                type: 'PUT',
     ...
     * @param datatables DataTable on which the calling should act upon
     */
    function getDataAddBatchUrl(datatables) {
        var url = getDataValue(datatables, 'add-batch-url');
        return url;
    }
     ...
     * @param idlist list of identifiers of the elements to remove
     */
    function getAddBatchUrl(datatables, idlist) {
        var url = getDataAddBatchUrl(datatables);
        return processUrl(datatables, url, idlist);
    }
    ...
    apiRegister('advanced.getAddBatchButton()', addBatchButton);
